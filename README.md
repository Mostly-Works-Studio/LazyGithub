...
- **One-click builds** — posts a `/deploy {branchName}` comment (or any custom command) to trigger a build workflow; the page scrolls to the new comment automatically
...
5. Clicking a Deploy button dispatches a `workflow_dispatch` event to the GitHub API via the background service worker; clicking Build posts a comment to the PR and navigates to it
...
| `host_permissions: https://api.github.com/*` | Validate your token on paste, dispatch workflows, and post build comments |
